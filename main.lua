...
if not BaseV then
  ...
  mod.exports.version = "0.1.93-beta.4"
...
-- v0.1.58-local test4:
-- DRAMALESS_SHAPE v2.x removed the old Stadium ROM/import/runtime modules.
-- StadiumRomMenu is loaded before the main V table below exists, so it needs
-- its own local-first require table. This lets StadiumRomMenu use this mod's
-- bundled StadiumInstall, StadiumRomPick, StadiumScreen, StadiumPack, etc.
local romMenuRequireCache = {}
local RomMenuV = {
  mod = mod,
  path = mod.path,
  voxelHostId = dramaticShapeId,
}
setmetatable(RomMenuV, { __index = BaseV })

function RomMenuV.require(name)
  ...
  if romMenuRequireCache[name] ~= nil then
    return romMenuRequireCache[name]
  end
  ...
  if source then
    local chunk, err = load(source, "@" .. mod.path .. "/" .. rel)
    assert(chunk, ("STADIUM_OVERWORLD_MODELS_LOCAL: %s did not compile: %s")
      :format(rel, tostring(err)))
    local value = chunk(RomMenuV)
    romMenuRequireCache[name] = value
    return value
  end
...
-- v0.1.61-local test7:
-- Keep Gen1Recomp's crash-prone per-mod Mod Manager options disabled, but
-- restore the legacy START -> OPTIONS hook after fixing StadiumRomMenu.poll().
local managerOptionsInstalled = false

do
  local ok, err = pcall(StadiumRomMenu.installOptionsHook, mod)
  if not ok then
    mod.log:warn("Stadium START -> OPTIONS hook failed: %s", tostring(err))
  else
    mod.log:info("Stadium START -> OPTIONS hook installed for test7")
  end
end
...
-- v0.1.84-local test44:
-- When we dynamically reload DRAMALESS v2's VoxelScene.lua, its renderer must
-- keep using DRAMALESS v2 core modules, not the legacy Stadium/Dramaless v1
-- copies bundled for StadiumMon/StadiumPack.  Prefer the host for renderer
-- modules and use local lib/ only for Stadium-specific modules.
local hostPreferredModules = {
  Mat4 = true,
  Voxel3D = true,
  ShadowMap = true,
  Shadows = true,
...
-- v0.1.57-local compatibility experiment:
-- DRAMALESS_SHAPE v2.x removed the old Stadium runtime modules
-- such as StadiumMon, StadiumPack, StadiumRig, Mat4, Voxel3D, etc.
-- For newer DRAMALESS builds, this companion mod first tries to load
-- Stadium runtime modules from its own lib/ folder, then falls back to
-- the detected Dramatic/Dramaless Shape host.
local localRequireCache = {}
...
function V.require(name)
  ...
  if hostPreferredModules and hostPreferredModules[name] and BaseV and type(BaseV.require) == "function" then
    return BaseV.require(name)
  end

  if localRequireCache[name] ~= nil then
    return localRequireCache[name]
  end
